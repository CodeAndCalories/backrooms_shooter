...
| `tools/test_scanner.js` | Lights Out (floor 18): the constraint-critical "dots add NO point lights" check (no PointLight/SpotLight in the dot source) + InstancedMesh/no-map/emissive material + per-instance-scale fade; drives the REAL `doScanPulse`/`castScanRay` over a grid (3D SPATTER — floor + wall + ceiling dots with a real vertical spread, forward floor-fan bias, LOS-gated monster reveal, slot-color routing); theme/darkness/flashlight/input/co-op-relay wiring. |
...
+ firer facing yaw `a` (so the forward floor-fan of the 3D SPATTER aims where they looked)
so teammates' dots paint in their colors; COSMETIC (monster reveal is per-machine, no host
authority; the pulse is a `Math.random` scatter — never needs determinism; the `a` field is
backward-compatible, a message without it just skips the forward fan). `run_won` (host → all): the 20th-floor finale boss died → the whole party gets
...
field. No gameplay state crosses beyond the gate timer. `escape_seq` (host → all, `{}`):
the AUTO-RUN chase ESCAPE ENDING — the first player to reach the end fires the scripted
"we made it" cinematic (gate slams behind, chaser slams into it, beat, auto-advance);
the host broadcasts so everyone plays the same deterministic sequence in sync. COSMETIC
(the actual advance is the host's authoritative `game_start` at the cinematic's end —
`exit_reached` on an auto-run floor routes to `triggerEscape` instead of advancing
immediately). Clients never self-advance (they wait for `game_start`); robust fall-through
to `advanceFloor` on any error → never soft-locks.
...
| Hotel Chase (AUTO-RUN) | main.js `AUTORUN_SPEED` (1.7× sprint — run pace), `AUTORUN_STRAFE_FRAC` 0.55 (A/D dodge), `CHASE_WALL_SPEED` **0.95× auto-run** (**the knife-edge knob** — wall pace vs run; ↓0.94 kinder / ↑0.96 brutal; 0.95 keeps it close on a clean run), `CHASE_RUN_GRACE` 1.8s (post-gate-open head start — wall frozen + can't catch), `CHASE_WALL_GRACE_DIST` 8 (visual head-start distance), `CHASE_CATCH_GAP` 2 (wall reach), `CHASE_GATE_HOLD` 2s (HOLD-E gate), `CHASE_RUNS` 15 / `CHASE_RUN_LEN` 15 / `CHASE_NARROW_RUNS` 3 / `CHASE_OBST_MIN` 0.2 / `CHASE_OBST_MAX` 0.5 (length, narrowing, obstacle density floor→peak), `CHASE_SIREN_PERIOD` 1.5s / `CHASE_SIREN_MIN` 0.42 / `CHASE_SIREN_MAX` 1.18 (alarm pulse), `CHASE_LIGHT_MULT` **2.7** / `CHASE_TURN_LIGHT_MULT` 1.55 (corridor brightness + turn/exit beacons), theme 17 fog (`fogColor` 0x2e0810 / `fogNear` 6 / `fogFar` 40) + `ambientIntensity` 0.14 / `darknessLevel` 0.3 (red-fog visibility), roar cadence 1.6–2.7s (`updateChaseWall`), ESCAPE ending `ESCAPE_TRIGGER_BACK` 9 / `ESCAPE_SEAL_BACK` 11 / `ESCAPE_SLAM_DUR` 0.5 / `ESCAPE_LUNGE_DUR` 0.42 / `ESCAPE_WALL_OVERSHOOT` 0.6 / `ESCAPE_TOTAL` 3.7 (the "we made it" cinematic — `triggerEscape`/`updateEscapeSequence`) | On-rails auto-run pace, the wall's knife-edge pace + head-start grace + reach, gate hold, corridor length/difficulty, the siren-pulse rhythm/depth, corridor-light brightness, the red-fog draw distance, and the escape-ending timing. The wall reuses the procedural blob-mass (`buildChaserMonster`), widened to fill the corridor; it advances in TRACK space (not pathfinding). Lights are corridor-aware (walk `chasePath`, brighter trail; dead-ends unlit) + siren-pulsed (intensity-only). Green EXIT arrow signs (`buildChaseExitSigns`) + wrecked-hotel furniture (`buildHotelObstacle` — chair/desk/cart/suitcases/wardrobe) are procedural no-map Standard (pinned), no lights. The chaser roar is layered + WaveShaper-distorted (`_chaserRoar`, audio.js). | 
