...
| Hotel Chase (AUTO-RUN) | main.js `AUTORUN_SPEED` (1.7× sprint — run pace), `AUTORUN_STRAFE_FRAC` 0.55 (A/D dodge), `CHASE_WALL_SPEED` 0.92× auto-run (**the knife-edge knob** — wall pace vs run; ↓0.90 kinder / ↑0.94 brutal), `CHASE_RUN_GRACE` 1.8s (post-gate-open head start — wall frozen + can't catch), `CHASE_WALL_GRACE_DIST` 8 (visual head-start distance), `CHASE_CATCH_GAP` 2 (wall reach), `CHASE_GATE_HOLD` 2s (HOLD-E gate), `CHASE_RUNS` 15 / `CHASE_RUN_LEN` 15 / `CHASE_NARROW_RUNS` 3 / `CHASE_OBST_MIN` 0.2 / `CHASE_OBST_MAX` 0.5 (length, narrowing, obstacle density floor→peak), `CHASE_SIREN_PERIOD` 1.5s / `CHASE_SIREN_MIN` 0.42 / `CHASE_SIREN_MAX` 1.18 (alarm pulse), `CHASE_LIGHT_MULT` 2.2 / `CHASE_TURN_LIGHT_MULT` 1.5 (corridor brightness + turn/exit beacons) | On-rails auto-run pace, the wall's knife-edge pace + head-start grace + reach, gate hold, corridor length/difficulty, the siren-pulse rhythm/depth, and corridor-light brightness. The wall reuses the procedural blob-mass (`buildChaserMonster`), widened to fill the corridor; it advances in TRACK space (not pathfinding). Lights are corridor-aware (walk `chasePath`) + siren-pulsed (intensity-only). |
